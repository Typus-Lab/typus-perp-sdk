...
export class BigVector<Element extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;
 ...
 readonly $typeName = BigVector.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::big_vector::BigVector<${PhantomToTypeStr<Element>}>`; readonly $typeArgs: [PhantomToTypeStr<Element>]; readonly $isPhantom = BigVector.$isPhantom;

 readonly id: ToField<UID>; readonly sliceCount: ToField<"u64">; readonly sliceSize: ToField<"u64">; readonly length: ToField<"u64">

 private constructor(typeArgs: [PhantomToTypeStr<Element>], fields: BigVectorFields<Element>, ) { this.$fullTypeName = composeSuiType( BigVector.$typeName, ...typeArgs ) as `${typeof PKG_V1}::big_vector::BigVector<${PhantomToTypeStr<Element>}>`; this.$typeArgs = typeArgs;

 this.id = fields.id;; this.sliceCount = fields.sliceCount;; this.sliceSize = fields.sliceSize;; this.length = fields.length; }

 static reified<Element extends PhantomReified<PhantomTypeArgument>>( Element: Element ): BigVectorReified<ToPhantomTypeArgument<Element>> { return { typeName: BigVector.$typeName, fullTypeName: composeSuiType( BigVector.$typeName, ...[extractType(Element)] ) as `${typeof PKG_V1}::big_vector::BigVector<${PhantomToTypeStr<ToPhantomTypeArgument<Element>>}>`, typeArgs: [ extractType(Element) ] as [PhantomToTypeStr<ToPhantomTypeArgument<Element>>], isPhantom: BigVector.$isPhantom, reifiedTypeArgs: [Element], fromFields: (fields: Record<string, any>) => BigVector.fromFields( Element, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => BigVector.fromFieldsWithTypes( Element, item, ), fromBcs: (data: Uint8Array) => BigVector.fromBcs( Element, data, ), bcs: BigVector.bcs, fromJSONField: (field: any) => BigVector.fromJSONField( Element, field, ), fromJSON: (json: Record<string, any>) => BigVector.fromJSON( Element, json, ), fromSuiParsedData: (content: SuiParsedData) => BigVector.fromSuiParsedData( Element, content, ), fromSuiObjectData: (content: SuiObjectData) => BigVector.fromSuiObjectData( Element, content, ), fetch: async (client: SuiClient, id: string) => BigVector.fetch( client, Element, id, ), new: ( fields: BigVectorFields<ToPhantomTypeArgument<Element>>, ) => { return new BigVector( [extractType(Element)], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData<Element extends PhantomReified<PhantomTypeArgument>>( typeArg: Element, data: SuiObjectData ): BigVector<ToPhantomTypeArgument<Element>> { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isBigVector(data.bcs.type)) { throw new Error(`object at is not a BigVector object`); }
