...
(async () => {
    ...
    // @ts-ignore
    let evts = dryrunRes.Transaction.events.filter((e: any) => (e.eventType ?? e.type ?? "").endsWith("ReleaseCollateralEvent"));
    console.log("DRY-RUN OK — ReleaseCollateralEvent:", JSON.stringify(evts[0]?.json ?? evts[0]?.parsedJson ?? evts[0], null, 2));
