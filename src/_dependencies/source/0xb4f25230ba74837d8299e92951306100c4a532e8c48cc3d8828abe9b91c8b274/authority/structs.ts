...
export function isAuthority(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::authority::Authority`; }
...
export class Authority implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::authority::Authority`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = Authority.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::authority::Authority`; readonly $typeArgs: []; readonly $isPhantom = Authority.$isPhantom;

 readonly whitelist: ToField<LinkedTable<"address", "bool">>

 private constructor(typeArgs: [], fields: AuthorityFields, ) { this.$fullTypeName = composeSuiType( Authority.$typeName, ...typeArgs ) as `${typeof PKG_V1}::authority::Authority`; this.$typeArgs = typeArgs;

 this.whitelist = fields.whitelist; }

 static reified( ): AuthorityReified { return { typeName: Authority.$typeName, fullTypeName: composeSuiType( Authority.$typeName, ...[] ) as `${typeof PKG_V1}::authority::Authority`, typeArgs: [ ] as [], isPhantom: Authority.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => Authority.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Authority.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => Authority.fromBcs( data, ), bcs: Authority.bcs, fromJSONField: (field: any) => Authority.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => Authority.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => Authority.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => Authority.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => Authority.fetch( client, id, ), new: ( fields: AuthorityFields, ) => { return new Authority( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData( data: SuiObjectData ): Authority { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isAuthority(data.bcs.type)) { throw new Error(`object at is not a Authority object`); }

 return Authority.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return Authority.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
